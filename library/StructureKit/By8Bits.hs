  ...
  if key < 128
    then
      if key < 64
        ...
        else By6Bits.insert key value b & second (\b -> By8Bits a b c d)
    else
      if key < 192
        then By6Bits.insert key value c & second (\c -> By8Bits a b c d)
        else By6Bits.insert key value d & second (\d -> By8Bits a b c d)
